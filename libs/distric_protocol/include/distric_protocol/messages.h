/**
 * @file messages.h
 * @brief Protocol Message Definitions - All Wire Formats
 * 
 * IMPORTANT: This file defines ALL WIRE FORMATS for network transmission.
 * It does NOT define internal application data structures.
 * 
 * Defines message structures used in the DistriC protocol:
 * - Raft consensus messages (RequestVote, AppendEntries, InstallSnapshot)
 * - Gossip protocol messages (Ping, Ack, MembershipUpdate)
 * - Task execution messages (Assignment, Result)
 * - Client API messages (Submit, Response)
 * 
