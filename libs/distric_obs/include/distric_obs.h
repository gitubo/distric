 ...
 *
 * =============================================================================
 * BEST-EFFORT SEMANTICS  (Gap #4 — explicit in public API)
 * =============================================================================
 *
 ...
 * METRICS SYSTEM
 *
 * Label cardinality enforcement (Gap #2):
 *   Labels are STRICTLY validated against per-metric allowlists.
 *   - When label_defs is non-NULL, only the listed keys and values are valid.
 *   - Any update using a key/value outside the allowlist returns
 *     DISTRIC_ERR_INVALID_LABEL and the update is silently dropped.
 *   - Registration is rejected with DISTRIC_ERR_HIGH_CARDINALITY when the
 *     theoretical combination count exceeds MAX_METRIC_CARDINALITY.
 ...
 * Set allowed_values=NULL to permit any value (use only for low-cardinality
 * labels). Otherwise, exactly these values are accepted at update time.
 */
typedef struct {
    char          key[MAX_LABEL_KEY_LEN];
...
/* Unlabelled convenience wrappers — lock-free, never return errors. */
...
/** Export Prometheus text. Caller must free *out_buffer. Thread-safe snapshot. */
 ...
 * STRUCTURED LOGGING
 *
 * Async mode concurrency model — MPSC ring buffer (Gap #1):
 *
 *   Producers (any thread calling LOG_* / log_write):
 ...
 *        This release store synchronises with the consumer's acquire load
 *        and makes all slot writes visible.
 *
 ...
 *        If the producer is delayed, the consumer skips and the tail does
 *        NOT advance — preserving ordering.
 *     3. Write entry to fd; mark slot EMPTY (release store).
 *     4. Advance rb->tail (release store) to reclaim the slot.
 *
 *   Memory ordering summary:
 *     - Producer publish:  release store on slot->state → makes content visible
 *     - Consumer observe:  acquire load on slot->state  → sees content
 *     - Consumer reclaim:  release store on rb->tail    → prevents wrap-around race
 *     - Drop check:        relaxed loads (approximate — acceptable for best-effort)
 * ========================================================================= */
 ...
 * DISTRIBUTED TRACING
 *
 * Adaptive sampling (Gap #3):
 *   Normal mode:      always_sample / (always_sample + always_drop)
 *   Backpressure mode (buffer > 75% full):
 *                     backpressure_sample / (backpressure_sample + backpressure_drop)
 *
 ...
 *   are safe and approximately free (~50ns).
 * ========================================================================= */
...
typedef struct trace_span_s {
    ...
    int         status;
    bool        sampled;
    void*       _tracer; /* internal — do not access */
} trace_span_t;
...
typedef struct { trace_id_t trace_id; span_id_t span_id; } trace_context_t;
...
distric_err_t trace_start_span_from_context(tracer_t* tracer, const trace_context_t* context, const char* operation, trace_span_t** out_span);
...
distric_err_t   health_register_component(health_registry_t* registry, const char* name, health_component_t** out_component);
distric_err_t   health_update_status(health_component_t* component, health_status_t status, const char* message);
health_status_t health_get_overall_status(health_registry_t* registry);
distric_err_t   health_export_json(health_registry_t* registry, char** out_buffer, size_t* out_size);
...
distric_err_t obs_server_init(obs_server_t** server, uint16_t port, metrics_registry_t* metrics, health_registry_t* health);
